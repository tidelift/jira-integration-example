...
from tidelift import (
    TideliftConfig,
    TideliftService,
    generate_unique_tidelift_issues_from_report,
)
...
tl_service = TideliftService(
    api_key=os.environ["TIDELIFT_API_KEY"], config=TideliftConfig(config["tidelift"])
)
...
tidelift_unique_issues = generate_unique_tidelift_issues_from_report(
    report, unique_key_fields=unique_key_fields
)

totals = {"processed": 0, "added": 0, "updated": 0}
...
for tidelift_unique_issue in tidelift_unique_issues:
    ...
    if existing_issue:
        jira_unique_field_service.update(
            existing_issue_id=existing_issue["id"], payload=renderer.to_json_update()
        )
